...
function renderPayments(){
  ...
  document.getElementById('cycle-grid').innerHTML=CYCLES.map((c,i)=>{
    ...
    return `<div class="cycle-card" style="${isCur?'border-color:var(--accent);border-width:2px':''}">
      ...
      <div style="font-size:11px;color:var(--dim);font-family:'DM Mono',monospace;margin-bottom:3px">GW${c.gw[0]}–${c.gw[1]}</div>
      <div style="font-size:10px;color:var(--dim);margin-bottom:6px">₦${c.fee.toLocaleString()}</div>
      <div style="font-family:'DM Mono',monospace;font-size:18px;font-weight:600;color:var(--text)">${paid}<span style="color:var(--dim);font-size:13px">/${c.players.length}</span></div>
...
function openCycleModal(idx){
  ...
  document.getElementById('modal-checklist').innerHTML=c.players.map(i=>{
    ...
    return `<div class="check-item" style="flex-wrap:wrap">
      ...
      <span style="font-size:11px;font-family:'DM Mono',monospace;color:var(--dim);margin-right:4px">₦${bal.toLocaleString()}</span>
  ...
  try{
    ...
    el.innerHTML=`<div class="tbl-wrap"><table>
      ...
      <tbody>${rows.map((r,i)=>`<tr>
        ...
        <td style="font-family:'DM Mono',monospace">${r.event_total}</td>
        <td style="font-family:'DM Mono',monospace;font-weight:600">${r.total}</td>
      </tr>`).join('')}</tbody>
    </table></div>`;
  }catch(e){
...
function renderSeasonTab(){
  ...
  const rows=CYCLES.map((c,idx)=>{
    ...
    return `<div style="display:flex;justify-content:space-between;align-items:flex-start;padding:10px 0;border-bottom:1px solid var(--border);gap:8px;flex-wrap:wrap">
      ...
      <span style="font-family:'DM Mono',monospace;font-size:13px;font-weight:700;color:var(--green);white-space:nowrap">${fmt(contribution)}</span>
    </div>`;
  }).filter(Boolean);

  const totalRow=`<div style="display:flex;justify-content:space-between;padding-top:10px;margin-top:4px;border-top:2px solid var(--border)"><span style="font-size:12px;font-weight:700;color:var(--muted)">Total</span><span style="font-family:'DM Mono',monospace;font-size:14px;font-weight:700;color:var(--text)">${fmt(pot)}</span></div>`;
...
function renderFinanceTab(){
  ...
  const tile=(label,val,color='var(--fpl-dark)')=>`
    <div style="background:var(--surface);border:1px solid var(--border);border-top:3px solid ${color};border-radius:8px;padding:1rem;text-align:center">
      ...
      <div style="font-family:'DM Mono',monospace;font-size:18px;font-weight:700;color:${color}">${fmt(val)}</div>
    </div>`;
  ...
  const row=(label,val,sub='',color='var(--text)')=>`
    <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--border)">
      ...
      <div style="font-family:'DM Mono',monospace;font-weight:700;color:${color}">${fmt(val)}</div>
    </div>`;
  ...
  const gwRows=[...state.gameweeks].reverse().map(g=>{
    ...
    return `<div style="display:flex;justify-content:space-between;align-items:flex-start;padding:10px 0;border-bottom:1px solid var(--border);gap:8px;flex-wrap:wrap">
      ...
      <div style="font-family:'DM Mono',monospace;font-size:13px;font-weight:700;flex-shrink:0">${fmt(total)}</div>
    </div>`;
  }).join('');
    ...
    ${rows.map(({p,i,badges})=>`
      <div class="card" style="margin-bottom:.75rem;cursor:pointer" onclick="openProfile(${i})">
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:${badges.length?'.75rem':'0'}">
          ...
          <div style="font-size:12px;color:var(--dim);font-family:'DM Mono',monospace">${badges.length} badge${badges.length!==1?'s':''}</div>
        </div>
  ...
  el.innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;margin-bottom:1rem">
      <div style="background:#f3e8ff;border-top:3px solid var(--accent);border-radius:8px;padding:.75rem;text-align:center">
        ...
        <div style="font-family:'DM Mono',monospace;font-size:clamp(11px,3vw,14px);font-weight:700;color:var(--accent)">₦${totalFees.toLocaleString()}</div>
      </div>
      <div style="background:#dcfce7;border-top:3px solid var(--green);border-radius:8px;padding:.75rem;text-align:center">
        ...
        <div style="font-family:'DM Mono',monospace;font-size:clamp(11px,3vw,14px);font-weight:700;color:var(--green)">₦${totalPrizes.toLocaleString()}</div>
      </div>
      <div style="background:#dbeafe;border-top:3px solid var(--blue);border-radius:8px;padding:.75rem;text-align:center">
        ...
        <div style="font-family:'DM Mono',monospace;font-size:clamp(11px,3vw,14px);font-weight:700;color:var(--blue)">₦${mib.toLocaleString()}</div>
      </div>
    </div>`; })()}
...
function renderEarningsChart(){
  ...
  el.innerHTML=players.map(p=>`
    <div style="display:grid;grid-template-columns:clamp(60px,25%,100px) 1fr clamp(44px,12%,64px);gap:6px;align-items:center;margin-bottom:9px">
      ...
      <span style="font-family:'DM Mono',monospace;font-size:11px;color:#374151">₦${Math.round(p.accumulated/1000)}k</span>
    </div>`).join('');
}
    ...
    <div class="tbl-wrap"><table>
      ...
      <tbody>${entries.map(e=>{
        ...
        return `<tr${podiumCls?' class="'+podiumCls+'"':''} onclick="openProfile(${e.idx})" style="cursor:pointer">
          ...
          ${hasPoints?`<td><span style="font-family:'DM Mono',monospace;font-weight:700;color:var(--fpl-dark)">${e.pts??'—'}</span></td>`:''}
...
function renderH2H(){
  ...
  const row=(label,getVal,fmt=v=>v)=>{
    ...
    return `<tr><td style="font-size:12px;color:var(--muted);padding:8px 4px;border-bottom:1px solid var(--border)">${label}</td>${rv.map(v=>`<td style="text-align:center;font-family:'DM Mono',monospace;font-size:13px;padding:8px 4px;border-bottom:1px solid var(--border);${hi(rv,v)}">${fmt(v)}</td>`).join('')}</tr>`;
  };
...
function openProfile(idx){
  ...
  document.getElementById('profile-content').innerHTML=`
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:1.25rem">
      <div style="background:#f3e8ff;border-top:3px solid var(--accent);border-radius:8px;padding:.75rem;text-align:center"><div style="font-size:10px;font-weight:700;color:var(--accent);margin-bottom:4px;text-transform:uppercase;letter-spacing:.04em">GW Earnings</div><div style="font-family:'DM Mono',monospace;font-size:13px;font-weight:700;color:var(--accent)">₦${p.accumulated.toLocaleString()}</div></div>
      <div style="background:#f5f5f5;border-top:3px solid var(--dim);border-radius:8px;padding:.75rem;text-align:center"><div style="font-size:10px;font-weight:700;color:var(--muted);margin-bottom:4px;text-transform:uppercase;letter-spacing:.04em">Withdrawn</div><div style="font-family:'DM Mono',monospace;font-size:13px;font-weight:700;color:var(--muted)">₦${withdrawn.toLocaleString()}</div></div>
      <div style="background:${bal>0?'#dcfce7':'#f5f5f5'};border-top:3px solid ${bal>0?'var(--green)':'var(--dim)'};border-radius:8px;padding:.75rem;text-align:center"><div style="font-size:10px;font-weight:700;color:${bal>0?'var(--green)':'var(--muted)'};margin-bottom:4px;text-transform:uppercase;letter-spacing:.04em">Balance</div><div style="font-family:'DM Mono',monospace;font-size:13px;font-weight:700;color:${bal>0?'var(--green)':'var(--dim)'}">₦${bal.toLocaleString()}</div></div>
    </div>
    ...
    ${history.length?history.map(g=>`<div class="gw-item" style="border-bottom:1px solid var(--border)">
      ...
      <span style="font-family:'DM Mono',monospace;font-size:13px;font-weight:700;color:var(--accent)">₦${g.amount.toLocaleString()}</span>
