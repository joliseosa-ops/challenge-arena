...
function renderFinanceTab(){
  ...
  // --- Outstanding per cycle (only cycles whose first GW has been played) ---
  const currentGW=state.gameweeks.length?state.gameweeks[state.gameweeks.length-1].gw:0;
  const cycleDebtors=CYCLES.map((c,idx)=>{
    const cp=state.cyclePayments[idx]||{};
    const unpaid=c.players.filter(i=>!isPaid(cp[i]));
    return {idx,c,unpaid,cp};
  }).filter(x=>x.c.gw[0]<=currentGW&&x.unpaid.length>0);
...
function renderDebtTracker(){
  ...
  const _currentGW=state.gameweeks.length?state.gameweeks[state.gameweeks.length-1].gw:0;
  const debtors=CYCLES.map((c,ci)=>{
    if(c.gw[0]>_currentGW) return null;
